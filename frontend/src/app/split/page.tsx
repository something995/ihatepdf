...
import { UploadCloud, File as FileIcon, Loader2, PlayCircle, Download } from 'lucide-react';
...
export default function SplitPage() {
    ...
    const handleSplit = async () => {
        if (!file) {
            setError("Please select a valid PDF file to split.");
            return;
        }
        ...
        try {
            ...
            setError(e.response?.data?.detail || "Failed to split file.");
            setStatus('idle');
        }
    };
    ...
    return (
        <div className="flex flex-col items-center pt-8 w-full max-w-4xl mx-auto space-y-12 min-h-[60vh]">
            ...
            <div className="text-center space-y-4">
                <h1 className="text-4xl md:text-5xl font-black">Split PDF</h1>
                <p className="text-slate-600 dark:text-slate-400 text-lg">Separate one page or a whole set for easy conversion into independent PDF files.</p>
            </div>
            ...
            <AnimatePresence mode="wait">
                {status === 'done' ? (
                    <motion.div
                        ...
                        className="w-full glass rounded-3xl p-12 text-center space-y-8 bg-blue-500/5 border-blue-500/20"
                    >
                        <div className="mx-auto w-20 h-20 bg-blue-500 rounded-full flex items-center justify-center text-white shadow-xl shadow-blue-500/40">
                            <PlayCircle size={36} />
                        </div>
                        <h2 className="text-3xl font-bold text-blue-600">The PDF has been split!</h2>

                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pt-4">
                            {downloads.map((f, i) => (
                                <a key={i} href={`${API_BASE}/download/${f}`} download className="group relative glass p-6 rounded-2xl flex flex-col items-center gap-3 hover:bg-blue-50 transition border border-transparent hover:border-blue-200">
                                    <Download className="w-8 h-8 text-blue-500 group-hover:scale-110 transition-transform" />
                                    <span className="font-semibold text-sm">Page {i + 1}</span>
                                </a>
                            ))}
                        </div>
                        ...
                        >
                            {!file ? (
                                ...
                                <div className="flex flex-col items-center gap-6">
                                    <FileIcon className="w-24 h-24 text-rose-500" />
                        ...
                        {file && (
                            <div className="flex justify-center pt-8">
                                ...
                                >
                                    ...
                                    {status === 'idle' ? 'Split PDF' : 'Splitting...'}
                                </button>
                            </div>
                        )}
                    </motion.div>
